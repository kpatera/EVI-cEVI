...
deviant_plus=function(new_cases, cum = FALSE, r_a=7, r=0.2, lag_max=30, method = "cEVI"){
  ...
  if(method=="cEVI"){
    ...
    ind=indic(cevi=cevi,cut = c_1, cases=cases[1:start_cases], method="cEVI")
    status=status(cases[1:start_cases],r)
    ...
    for (i in (start_cases+1): length(cases)){
      ...
      for (l in c_s) {
        for (j in lag_s) {
          ...
          for(k in (j+1):(length(case_t)-(j+1))){
            enu=mean(case_t[(k+1):(k+j)]-case_t[(k):(k-(j-1))],na.rm = T)
            den1=sd(case_t[(k):(k-(j-2))])^2/(length(case_t[(k):(k-(j-2))]))
            den2=sd(case_t[(k+1):(k+j)])^2/(length(case_t[(k+1):(k+j)]))
      ...
      ind_n=indic(cevi = cevi,cut = c_n, cases = case_t, method="cEVI") #
