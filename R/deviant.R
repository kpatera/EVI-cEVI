...
#' @param combo when multiple indeces are selected, choose whether to combine indeces via conjunction (and, &) or via disjuction (or |)
#'
#' @examples
#' data("Italy")
...
deviant=function(new_cases, cum = FALSE, r_a=7, r=0.2, lag_max=40, method="EVI"){
  ...
  if(method=="cEVIplus"){
    ...
    cevi=cEVI_fun(cases = cases[1:(start_cases)],lag_n = lag_1, c_n = c_1)
    ind_cevi=indic(cevi=cevi, cases=cases[1:start_cases], method="cEVI")
    ...
    for (i in (start_cases + 1):length(cases)) {
      case_t = cases[1:i]
      lag_s_cevi=seq(lag_1_cEVI,min(lag_max,(i-i/2-4)),by= 2)
      c_s_cevi=seq(0.001,0.5, 0.06)
      lag_s_evi = seq(lag_1, min(lag_max, (length(case_t) - 1)),
                      1)
      c_s_evi = seq(0.01, 0.5, 0.01)
      all_lag = NA
      all_cut = NA
      all_se = NA
      all_sp = NA
      ...
      for (l in c_s) {
        for (j in lag_s) {
          ...
          all_lag[[length(all_lag) + 1]] <- j
          all_cut[[length(all_cut) + 1]] <- l
          all_se[[length(all_se) + 1]] <- evicut_t_ceviplus[[1]]
          all_sp[[length(all_sp) + 1]] <- evicut_t_ceviplus[[2]]
        }
      }
      ...
      roll_n = rollsd(cases[1:i], lag_n)
      ev_n = evi(roll_n) ###
      ind_n = indic(evi = ev_n, cut = c_n, cases = case_t,method="cEVIplus") ###
      evicut_n = evifcut(evi = ev_n, cases = case_t, cut = c_n, r = r,method="cEVIcplus") ###
      cevi=cEVI_fun(cases = cases[1:i],lag_n = lag_n, c_n = c_n) #
      ind_n=indic(cevi = cevi, cases = case_t, method="cEVI") #
      evicut_n=evifcut(cevi = cevi, cases = case_t, r = r, method="cEVI") #
      roll = c(roll, roll_n[i])
