...
deviant_cEVI=function(new_cases, cum = FALSE, r_a=7, r=0.2, lag_max=30, method = "cEVI"){
  #source("mova.r")
  #source("medvol.r")
  #source("evi.r")
  #source("evifcut.r")
 ...
 start_time = Sys.time()
  ...
  status=status(cases[1:start_cases],r) # Status condition has to change. Remove equal.
  ...
  for (i in (start_cases+1): length(cases)){
    ...
    if(method=="cEVI"){
      ...
      for (l in c_s) {
        lag_s=seq(lag_1,min(lag_max,(i-i/2-2)), 3)
        for (j in lag_s) {
          # roll_t <- rollsd(case_t,j)
          #  ev_t <- evi(roll_t)
          cevi <- rep(NA, length(cases))
          for(k in (j+1):(length(cases)-(j))){
            enu=mean(cases[(k+1):(k+j)]-cases[k:(k-j+1)],na.rm = T)
            den1=sd(cases[k:(k-j)])^2/(length(cases[k:(k-j)]))
